...
filename = r"C:\Users\lexkr\Downloads\2secpelletretrieval\Fluorescence267_processed.csv"
...
timestamps = list(pulse2["TimeStampAdjusted"])
del timestamps [14]
...
ax_heatmap.set_title(filename[-17:-14] + "     Poke", fontsize=20)
...
ax_line = fig.add_axes([0.12, 0.1, 0.63, 0.2])  # left, bottom, width, height
ax_line.plot(np.nanmean(Fluorescence_corrected_matrix, axis=0), color='blue', lw=2)
ax_line.set_xlabel('Seconds from Event', fontsize=14)
ax_line.set_ylabel('Z-score', fontsize=14)
...
timestamps = list(pulse1["TimeStampAdjusted"])
del timestamps [14]
...
ax_heatmap.set_xticklabels("")
ax_heatmap.grid(False)  # Remove grid lines
ax_heatmap.set_title(filename[-17:-14] + "     Pellet", fontsize=20)
